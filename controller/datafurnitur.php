...
class furnitur {
    ...
    public function InsertFurnitur ($id,$no_brg, $model, $merk, $status, $tgl, $th_beli, $jumlah, $stn_hrg, $ttl_hrg){
        try{
            $insetkb = $this->db->prepare("INSERT INTO crudfurnitur (id,no_brg,model,merk,status,tgl_input,th_beli,jumlah,stn_hrg,ttl_hrg) VALUES(:id,:no_brg,:model,:merk,:status,:tgl_input,:th_beli,:jumlah,:stn_hrg,:ttl_hrg)");
    ...
    public function Tampiladminfurnitur ($query){
        ...
        while($row = $query->fetch(PDO::FETCH_ASSOC))
        {
            ...
            <td>
                <a href="controller/class_hapus.php?id_furnitur='<?php echo $row['id'] ?>'" onclick='return confirm("apakah anda ingin menghapus?");' class='btn btn-danger'><span class='bi bi-trash'></span></a>
            </td>
            <td> 
